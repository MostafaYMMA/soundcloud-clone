  ...
  group('EngagementService playlist methods', () {
    test('createPlaylist returns playlist data', () async {
      when(() => mockDio.post(any(), data: any(named: 'data'))).thenAnswer(
        (_) async => _res({
          'data': {'id': 'playlist-1', 'name': 'New Playlist'},
        }),
      );

      final result = await sut.createPlaylist(name: 'New Playlist');
    ...
    test('getPlaylist returns playlist data', () async {
      when(() => mockDio.get(any())).thenAnswer(
        (_) async => _res({
          'data': {'id': 'playlist-1', 'name': 'My Playlist'},
        }),
      );
    ...
    test('updatePlaylist completes on success', () async {
      ...
      await expectLater(
        sut.updatePlaylist(playlistId: 'playlist-1', name: 'Updated'),
        completes,
      );
    });
    ...
    test('addTrackToPlaylist completes on success', () async {
      ...
      await expectLater(
        sut.addTrackToPlaylist(playlistId: 'playlist-1', trackId: 'track-1'),
        completes,
      );
    });
